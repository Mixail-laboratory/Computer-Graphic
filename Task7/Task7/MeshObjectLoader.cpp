#include "MeshObjectLoader.hpp"


using namespace std;

GLuint MeshObjectLoader::load(const QString &filename) {
    ...
    for (size_t i = 0; i < N_coord; ++i) {
        if (coord[i][0] != '#') {
            ...
            {
                Vertex v1, v2, v3;
                sscanf(coord[i].c_str(), "f %i/%i/%i %i/%i/%i %i/%i/%i", &v1.v_i, &v1.vt_i, &v1.vn_i, &v2.v_i, &v2.vt_i,
                       &v2.vn_i, &v3.v_i, &v3.vt_i, &v3.vn_i);
...
GLuint MeshObjectLoader::draw(GLfloat offset) {
    ...
    for (size_t i = 0; i < N_faces; ++i) {
        ...
        glVertex3f(vertex[f.v[0].v_i - 1].x(), vertex[f.v[0].v_i - 1].y(),
                   vertex[f.v[0].v_i - 1].z()); // читаем текстурную вершину из файла
